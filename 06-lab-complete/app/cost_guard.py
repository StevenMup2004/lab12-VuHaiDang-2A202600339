"""Monthly budget guard with optional Redis-backed state."""
...
class CostGuard:
    ...
    def usage_key(self, user_id: str) -> str:
        return f"budget:{self.month_prefix()}:{user_id}"
    ...
    def record_usage(self, user_id: str, input_tokens: int, output_tokens: int) -> float:
        ...
        if self._redis is not None:
            ...
            self._redis.expire(key, self.seconds_until_next_month())
