...
def run_checks():
    ...
    if os.path.exists(main_py):
        ...
        results.append(check("Rate limiter integrated",
                             "RateLimiter" in content and "RATE_LIMIT_PER_MINUTE" in content))
        results.append(check("Cost guard integrated",
                             "CostGuard" in content and "MONTHLY_BUDGET_USD" in content))
        results.append(check("Redis stateless support",
                             "REDIS_URL" in content and "_connect_redis" in content))
        results.append(check("Graceful shutdown hooks",
                             "_handle_signal" in content and "SIGTERM" in content))
    else:
